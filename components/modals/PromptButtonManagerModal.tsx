        ...
        >
            <div className="space-y-4">
                ...
                <div className="bg-black/20 p-2 rounded border border-white/10 max-h-60 overflow-y-auto custom-scrollbar space-y-2">
                    {promptButtons.length === 0 && <p className="text-center text-zinc-600 dark:text-zinc-400 py-4 italic text-xs">No buttons created yet.</p>}
                        ...
                        >
                            ...
                            <div className="flex-grow min-w-0 mr-2">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className={`text-[10px] uppercase px-1.5 py-0.5 rounded border ${btn.action === 'send' ? 'bg-emerald-100 text-emerald-800 border-emerald-200 dark:border-emerald-500/30 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-indigo-100 text-indigo-800 border-indigo-200 dark:border-indigo-500/30 dark:bg-indigo-900/30 dark:text-indigo-400'}`}>
                                        {btn.action}
                                    </span>
                                </div>
                                <p className="text-xs text-zinc-600 dark:text-zinc-400 truncate">{btn.content}</p>
                            </div>
                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <Button 
                                    variant="ghost"
                                    onClick={() => handleEdit(btn)} 
                                    className="p-1.5 text-emerald-400 hover:bg-emerald-500/20 h-auto"
                                    icon={<PencilIcon className="w-3.5 h-3.5"/>}
                                />
                                <Button 
                                    variant="ghost"
                                    onClick={(e) => handleDelete(e, btn.id)} 
                                    className="p-1.5 text-red-400 hover:bg-red-500/20 h-auto"
                ...
                <div className="bg-black/30 p-4 rounded border border-white/10 relative">
                    <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3 flex items-center">
                        {editingId ? <PencilIcon className="w-3 h-3 mr-1.5 text-emerald-400"/> : <PlusIcon className="w-3 h-3 mr-1.5 text-green-400"/>}
                        {editingId ? "Edit Button" : "Create New Button"}
                    </h4>
                    
                    <div className="grid grid-cols-3 gap-3 mb-3">
                        <div className="col-span-2">
                            <label className="block text-[10px] text-zinc-600 dark:text-zinc-400 mb-1">Label</label>
                            <Input 
                                type="text" 
                                value={label} 
                        ...
                        <div>
                            <label className="block text-[10px] text-zinc-600 dark:text-zinc-400 mb-1">Type</label>
                            <Select 
                                value={action} 
                    ...
                    <div className="mb-3">
                        <label className="block text-[10px] text-zinc-600 dark:text-zinc-400 mb-1">Content / Prompt</label>
                        <Textarea 
                            value={content} 
