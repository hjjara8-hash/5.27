...
const ArchiverModal: React.FC = memo(() => {
    ...
    const footerButtons = (
        <div className="flex flex-col-reverse sm:flex-row gap-2 w-full sm:w-auto sm:justify-end">
            ...
            {isProcessing && !isPaused && (
                <Button variant="primary" onClick={pauseArchiving} icon={<PauseIcon className="w-4 h-4" />} className="bg-amber-600 hover:bg-amber-500 shadow-amber-500/20 w-full sm:w-auto">
                    Pause
                </Button>
            )}
            ...
            {chapters.length > 0 && !isProcessing && !reviewMode && (
                <>
                    <Button variant="primary" onClick={handleSaveToStory} icon={<BookOpenIcon className="w-4 h-4" />} className="bg-cyan-600 hover:bg-cyan-500 shadow-cyan-500/20 w-full sm:w-auto">
                        Save to Story
                    </Button>
    ...
    return (
        <BaseModal
            ...
            headerIcon={<ArchiveBoxIcon className="w-5 h-5 text-emerald-500" />}
        ...
        >
            <div className="space-y-4">
                {/* Info Header */}
                {!reviewMode && (
                    <div className="bg-emerald-50 dark:bg-emerald-900/20 p-3 rounded border border-emerald-200 dark:border-emerald-500/30 text-emerald-700 dark:text-emerald-200 text-sm flex justify-between items-start">
                        ...
                        <div className="flex gap-2">
                            {chapters.length > 0 && !isProcessing && !isPaused && (
                                <Button variant="ghost" size="sm" onClick={resetArchiver} className="text-emerald-600 dark:text-emerald-300 underline whitespace-nowrap p-0 h-auto hover:bg-transparent">Clear All</Button>
                            )}
                            <Button variant="ghost" size="sm" onClick={handleOpenStoryManager} className="text-emerald-600 dark:text-emerald-300 underline font-bold whitespace-nowrap p-0 h-auto hover:bg-transparent">Manage Chapters</Button>
                        </div>
                    </div>
                )}
                ...
                {!isProcessing && !reviewMode && chapters.length === 0 && (
                    <div className="grid grid-cols-2 gap-4 animate-fade-in">
                        <div>
                            <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 mb-1">User Name (for Story)</label>
                            <Input 
                                type="text" 
                                value={userName} 
                        ...
                        <div>
                            <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 mb-1">AI Character Name</label>
                            <Input 
                                type="text" 
                                value={charName} 
                ...
                {reviewMode && !isProcessing && (
                    <div className="animate-fade-in-right">
                        <div className="flex justify-between items-center mb-3">
                            <h3 className="text-sm font-bold text-gray-900 dark:text-gray-300">Select Chapters to Archive</h3>
                            <div className="space-x-2 flex">
                                <Button variant="ghost" size="sm" onClick={() => setAllChunksSelection(true)} className="text-[10px] bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 px-2 py-1 h-auto hover:bg-emerald-200 dark:hover:bg-emerald-900/50">Select All</Button>
                                <Button variant="ghost" size="sm" onClick={() => setAllChunksSelection(false)} className="text-[10px] bg-gray-100 dark:bg-white/5 text-gray-600 dark:text-gray-400 px-2 py-1 h-auto hover:bg-gray-200 dark:hover:bg-white/10">None</Button>
                            </div>
                        </div>
                        <div className="max-h-80 overflow-y-auto custom-scrollbar border border-gray-200 dark:border-white/10 rounded bg-gray-50 dark:bg-black/20 p-2 space-y-2">
                            {chunks.map((chunk, idx) => (
                                <div 
                                    key={idx} 
                                    className={`flex items-start p-2 rounded cursor-pointer border transition-colors ${chunk.selected ? 'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-500/30' : 'bg-transparent border-gray-200 dark:border-white/5 opacity-60'}`}
                                ...
                                >
                                    ...
                                    <div className="flex-grow">
                                        <div className="flex justify-between">
                                            <span className={`text-sm font-bold ${chunk.selected ? 'text-emerald-600 dark:text-emerald-300' : 'text-gray-500'}`}>
                                                Chapter {chunk.displayId}
                                            </span>
                                            <span className="text-[10px] text-gray-500">{chunk.msgCount} msgs</span>
                                        </div>
                                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 italic line-clamp-1">{chunk.previewText}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                {(isProcessing || isPaused) && (
                    <div className="space-y-2">
                        ...
                        <div className="w-full h-2 bg-black/40 rounded-full overflow-hidden">
                            <div 
                                className={`h-full transition duration-300 ease-out ${isPaused ? 'bg-amber-500' : 'bg-indigo-500'}`}
                ...
                {isPaused && (
                    <div className="bg-amber-50 dark:bg-amber-900/20 p-3 rounded border border-amber-200 dark:border-amber-500/30 animate-fade-in">
                        <label className="block text-xs font-bold text-amber-700 dark:text-amber-300 mb-2 uppercase tracking-wider">
                        ...
                        <Select
                            ...
                            options={MODEL_DEFINITIONS.map(m => ({ value: m.id, label: m.name }))}
                        />
                        <p className="text-[10px] text-amber-600 dark:text-amber-400/60 mt-1">
                ...
                {chapters.length > 0 && !reviewMode && (
                    <div className="mt-4 max-h-64 overflow-y-auto custom-scrollbar space-y-3 bg-gray-50 dark:bg-black/20 p-2 rounded border border-gray-200 dark:border-white/5">
                        {chapters.map((chapter, idx) => {
                            ...
                            return (
                                <div key={idx} className={`p-3 rounded border animate-fade-in ${chapter.isError ? 'bg-red-50 dark:bg-red-900/10 border-red-200 dark:border-red-500/30' : 'bg-white dark:bg-white/5 border-gray-200 dark:border-white/5'}`}>
                                    <div className="flex justify-between items-start mb-2">
                                        <h4 className={`text-sm font-bold ${chapter.isError ? 'text-red-600 dark:text-red-300' : 'text-emerald-600 dark:text-emerald-300'}`}>
                                            Chapter {chapter.chapterNumber ?? (idx + 1)}: {chapter.title}
                                        </h4>
                                        <span className="text-[10px] text-gray-500">{chapter.time_range}</span>
                                    </div>
                                    <p className="text-xs text-gray-700 dark:text-gray-300 mb-2 leading-relaxed">{chapter.narrative}</p>
                                    ...
                                    {chapter.key_quotes && chapter.key_quotes.length > 0 && !chapter.isError && (
                                        <div className="bg-gray-100 dark:bg-black/20 p-2 rounded">
                                            ...
                                            <ul className="list-disc list-inside text-[10px] text-gray-600 dark:text-gray-400 italic">
