    ...
    return (
      <div className={`flex items-center py-1 px-2 hover:bg-white/5 rounded-md cursor-pointer ${level === 0 ? '' : 'ml-4'}`} onClick={() => onToggleFile(node.path)}>
        <Switch
          ...
          className="mr-3 rounded border-gray-600 bg-black/30 text-emerald-500 focus:ring-emerald-500 focus:ring-offset-gray-900"
        />
    ...
    <div className={`${level === 0 ? '' : 'ml-4'}`}>
      ...
      >
        ...
        <input
          ...
          className="mr-3 rounded border-gray-600 bg-black/30 text-emerald-500 focus:ring-emerald-500 focus:ring-offset-gray-900"
        />
        <FolderOpenIcon className="w-4 h-4 mr-2 text-emerald-400 flex-shrink-0" />
...
const GitHubImportModal: React.FC<GitHubImportModalProps> = memo(({ isOpen, onClose, onImport }) => {
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-white/10 p-6 rounded-lg shadow-2xl w-full sm:max-w-2xl max-h-[90vh] flex flex-col text-gray-200 animate-modal-open"
        onClick={(e) => e.stopPropagation()}
      >
