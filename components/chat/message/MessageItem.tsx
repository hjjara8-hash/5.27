...
const MessageItemComponent: React.FC<MessageItemProps> = ({ message, canRegenerateFollowingAI, highlightTerm, onEnterReadMode, isContentExpanded, isThoughtsExpanded, onToggleExpansion, isLatestMemoryUpdate }) => {
  ...
  if (message.isTimeMarker) {
      ...
      return (
          <div className="flex justify-center items-center my-6 w-full opacity-70 select-none pointer-events-none animate-message-enter">
              <span className="text-[10px] sm:text-xs font-medium text-gray-500 dark:text-gray-400 bg-black/20 px-3 py-1 rounded-full border border-white/5 flex items-center shadow-sm">
  ...
  if (message.role === ChatMessageRole.SYSTEM) {
    ...
    return ( <div className="flex justify-center items-center my-3 w-full animate-message-enter" id={`message-item-${message.id}`}><div className="text-center text-xs text-gray-400 bg-black/25 px-4 py-1.5 rounded-full shadow-inner flex items-center gap-2 border border-white/5">{isGithubMessage && <GitHubIcon className="w-4 h-4 flex-shrink-0" />}<p>{message.content}</p></div></div>);
  }
  ...
  if (message.isSystemReminder) {
      ...
      return (
          <div 
            className={`flex justify-center items-center my-2 w-full group/divider relative animate-message-enter transition duration-200 ${isSelectionModeActive ? 'cursor-pointer' : ''} ${isSelectedAnchor ? 'bg-emerald-50 dark:bg-emerald-500/10 py-3 rounded-xl border border-emerald-200 dark:border-emerald-500/20 my-3' : ''}`} 
            id={`message-item-${message.id}`}
            onClick={handleSelectionClick}
          >
              <div className="relative flex items-center">
                  <div 
                      className={`flex items-center justify-center p-2 rounded-full bg-white/5 border border-white/5 transition cursor-help ${isLatestMemoryUpdate ? 'border-cyan-500/50 shadow-[0_0_10px_rgba(6,182,212,0.3)] bg-cyan-900/20' : 'hover:border-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-500/10'}`}
                  >
                      {isLatestMemoryUpdate ? (
                          <BrainIcon className="w-4 h-4 text-cyan-400 animate-pulse" />
                      ) : (
                          <ArrowPathIcon className="w-3.5 h-3.5 text-gray-500 group-hover/divider:text-emerald-600 dark:group-hover/divider:text-emerald-400 transition-colors" />
                      )}
                  </div>
                  ...
                  {!isSelectionModeActive && (
                      <Button
                          ...
                          variant="ghost"
                          size="none"
                          className="absolute left-full ml-3 p-2 text-gray-500 hover:text-red-400 bg-black/60 rounded-full border border-white/10 opacity-0 group-hover/divider:opacity-100 transition duration-200 hover:bg-red-900/30 hover:border-red-500/50 transform -translate-x-2 group-hover/divider:translate-x-0 z-30"
              ...
              <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 w-auto max-w-[90vw] sm:max-w-sm bg-black/90 border border-white/10 rounded-lg p-3 shadow-xl opacity-0 group-hover/divider:opacity-100 transition-opacity pointer-events-none group-hover/divider:pointer-events-auto z-20">
  ...
  let bubbleClasses = '';
  if (isUser) {
      bubbleClasses = 'bg-zinc-100 dark:bg-[#27272a] border border-transparent dark:border-white/5 rounded-2xl px-5 py-4 text-zinc-900 dark:text-zinc-200 shadow-sm';
  } else if (isError) {
      bubbleClasses = 'bg-red-900/30 border border-red-500/50 text-red-100 shadow-lg shadow-red-900/20 rounded-2xl rounded-bl-sm';
  } else {
      bubbleClasses = 'bg-white dark:bg-[#18181b] border border-zinc-200 dark:border-white/5 rounded-2xl px-5 py-4 text-zinc-900 dark:text-zinc-300 shadow-sm';
  }
  ...
  return (
    <div 
        id={`message-item-${message.id}`} 
        className={`group flex items-start mb-4 w-full relative transition-colors duration-200 animate-message-enter ${isSelected ? 'bg-emerald-50 dark:bg-emerald-500/10 rounded-xl -mx-2 px-2 py-2 border border-emerald-200 dark:border-emerald-500/20' : ''} ${isSelectionModeActive ? 'cursor-pointer' : ''} ${layoutClasses} message-item-root`} 
        onClick={handleSelectionClick} 
        role="listitem"
    >
      {!isUser && isSelectionModeActive && (
          <div className="flex-shrink-0 self-center px-2 flex items-center space-x-2">
              {selectionOrder > 0 && (
                  <span className="text-xs font-bold text-emerald-700 dark:text-emerald-300 bg-emerald-100 dark:bg-emerald-500/20 rounded-full w-5 h-5 flex items-center justify-center shadow">
                      {selectionOrder}
                  </span>
              )}
              <input 
                ...
                className="w-4 h-4 text-emerald-600 dark:text-emerald-500 bg-white dark:bg-black/30 border-gray-300 dark:border-white/20 rounded focus:ring-emerald-500 focus:ring-offset-white dark:focus:ring-offset-black cursor-pointer pointer-events-none" 
            ...
            <div className={`px-5 py-4 ${bubbleClasses} relative w-full sm:w-auto mt-1 min-w-[100px]`}>
                <div className="sticky top-2 z-20 flex w-full h-0 pointer-events-none">
                    <div className="absolute top-0 w-fit ltr:-left-3 rtl:-right-3 flex items-start opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity duration-200 pointer-events-auto" aria-label="Message actions">
                        <div className="flex items-center gap-1 p-1 bg-black/60 rounded-lg border border-white/10 shadow-lg scale-90 hover:scale-100 origin-top">
                    ...
                    {!isError && (
                        <Button
                            ...
                            variant="ghost"
                            size="none"
                            className={`absolute top-0 ltr:-right-3 rtl:-left-3 p-2 opacity-0 group-hover:opacity-100 transition-opacity focus:opacity-100 rounded-full hover:bg-black/20 ${isSelectionModeActive ? 'hidden' : ''} pointer-events-auto`}
                        >
                ...
                <MessageAttachments 
                    ...
                    isSelectionModeActive={isSelectionModeActive} 
                />
                {groundingChunks && groundingChunks.length > 0 && (<div className="mt-4 pt-3 border-t border-black/10 dark:border-white/10"><h4 className="text-[10px] font-bold uppercase tracking-widest mb-2 opacity-60 flex items-center"><MagnifyingGlassIcon className="w-3 h-3 mr-1.5" />{t.sources}</h4><ul className="grid grid-cols-1 gap-1">{groundingChunks.map((chunk: any, index: number) => (<li key={index} className="text-xs truncate"><a href={chunk.web.uri} target="_blank" rel="noopener noreferrer" title={chunk.web.uri} className="text-emerald-600 dark:text-emerald-400 hover:underline flex items-center bg-black/5 dark:bg-black/20 px-2 py-1 rounded border border-black/5 dark:border-white/5"><span className="mr-2 opacity-50">{index + 1}.</span> {chunk.web.title || chunk.web.uri}</a></li>))}</ul></div>)}</>
                    ...
                    <>
                        ...
                        <div className="flex items-center gap-1 group/seed">
                            ...
                            <Button
                                ...
                                variant="ghost"
                                size="none"
                                className="opacity-0 group-hover/seed:opacity-100 text-teal-400 hover:text-teal-200 p-0.5 rounded hover:bg-teal-900/30 transition"
                                title={t.lockSeed}
                            >
