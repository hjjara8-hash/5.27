        ...
        >
            ...
            <div className={`sticky top-0 w-full flex justify-end px-1 z-50 pointer-events-none transition-opacity duration-300 -mb-10 ${showScrollTop ? 'opacity-100' : 'opacity-0'}`}>
                <Button
                    ...
                    variant="ghost"
                    size="icon"
                    className="rounded-full bg-white dark:bg-zinc-800 text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-50 dark:hover:bg-zinc-700 border border-gray-200 dark:border-white/10 shadow-lg pointer-events-auto transform translate-y-1"
                    title="Scroll to Top"
                >
            ...
            {currentChatSession ? (
                        ...
                        {!isLoading && !isCharacterModeActive && showContinueFlowButton && visibleMessages.length > 0 && visibleMessages[visibleMessages.length - 1].role === 'model' && (
                            <div className="relative z-10 mt-2 mb-2 px-4 flex flex-col items-start gap-4 animate-fade-in">
                                ...
                                <Button 
                                    ...
                                    className="flex items-center gap-2 py-2.5 px-6 rounded-full border-emerald-200 dark:border-emerald-500/30 text-emerald-700 dark:text-emerald-400 bg-white dark:bg-emerald-500/5 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 transition-all shadow-md group border-2"
                                >
                ...
                ) : ( <div className="text-center text-gray-500 dark:text-gray-400 italic mt-10">{isCharacterMode && characters.length === 0 ? "Add some characters and start the scene!" : (isCharacterMode ? "Select a character to speak." : t.noChats)}</div>)
            ) : ( <div className="text-center text-gray-500 dark:text-gray-400 italic mt-10">{t.noChats}</div>)}
            ...
            <div className={`sticky bottom-4 w-full flex justify-end px-1 z-40 pointer-events-none transition-opacity duration-300 -mt-10 ${showScrollBottom ? 'opacity-100' : 'opacity-0'}`}>
                <Button
                    ...
                    variant="primary"
                    size="icon"
                    className="rounded-full shadow-lg pointer-events-auto transform -translate-y-1"
