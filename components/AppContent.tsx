...
const AppContent: React.FC = memo(() => {
  ...
  return (
    <div className="flex h-[100dvh] antialiased bg-zinc-50 dark:bg-[#0e0e11] text-gray-900 dark:text-zinc-100 overflow-hidden w-screen">
      
        <div className={`fixed inset-y-0 left-0 z-30 transform transition-transform duration-300 ease-[cubic-bezier(0.25,1,0.5,1)] ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} w-[85vw] sm:w-72`}>
          <Sidebar />
        </div>

        {isSidebarOpen && <div className="fixed inset-0 z-20 bg-black/50" onClick={closeSidebar} aria-hidden="true" />}
        
        <main className={`relative z-10 flex-1 flex flex-col overflow-y-auto transition duration-300 ease-in-out ${isSidebarOpen ? 'md:ml-72' : ''} ${isAudioBarVisible ? 'pt-[76px]' : ''}`}>
        ...
        <div className='absolute'>
            <Suspense fallback={null}>
                {isAudioBarVisible && (
                    <div className={`fixed top-0 left-0 right-0 z-30 transition duration-300 ease-in-out ${isSidebarOpen ? 'md:left-72' : ''}`}>
                      <AdvancedAudioPlayer
